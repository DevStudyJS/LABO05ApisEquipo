...
const fetchSinglePost = async (id: number): Promise<void> => {
  ...
  try {
    ...
    console.log('Datos que te da el back: ', data);
...
const fetchCommentsByPost = async (postId: number): Promise<void> => 
{
  ...
  console.log(`[LAB 5] PostId recibido ${postId}`, "color: orange; font-weight: bold;");
  
  try {
    // 2. [PETICIÓN]: Crea una constante 'response'.
    // Usa 'fetch' con backticks para unir API_URL + /posts/ + postId + /comments.
    const respuesta = await fetch(`${API_URL}/posts/${postId}/comments`);
  ...
  } catch (error) 
  {
    ...
    console.error("Fallo en traer los comentarios:", error);
  }
};
